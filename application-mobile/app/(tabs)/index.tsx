...
const darkTheme = {
  ...
  itemBg: '#1a2622',
  upcoming: '#75787eff', // Gris par défaut, tnajjem tbadlou hna
};
...
const lightTheme = {
  ...
  itemBg: '#e8f5ef',
  heroBg: '#e0f2ea',
  upcoming: '#737577ff', // Gris par défaut clair, tnajjem tbadlou hna
};
          ...
          .then(actData => {
            if (actData.success) {
              ...
              const filtered = (actData.data || []).filter((a: any) => {
                const actDate = new Date(a.dateCreation || a.createdAt).getTime();
                // On garde seulement si: date est aujourd'hui/future OU pas plus vieille que 24h
                return (now - actDate) < ONE_DAY_MS;
              }).sort((a: any, b: any) =>
                new Date(b.dateCreation || b.createdAt).getTime() - new Date(a.dateCreation || a.createdAt).getTime()
              );
